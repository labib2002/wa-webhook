...
//   1) SELECT by key: found -> return it deduped, nothing is sent. Exception:
//      a 'failed' row never reached Meta, so lib/templateSend.js re-sends onto
//      it rather than reporting a delivery that never happened.
...
//
// Degradation: on a DB error in these helpers the send proceeds WITHOUT
// dedupe, exactly as pre-feature, rather than 500ing. That is the dangerous
// path (it can double-send a billable template), so the warning re-arms on an
// interval instead of latching once per process.

const { makeWarner } = require('./warn');

const warnDisabled = makeWarner('idempotency disabled, sending without dedupe');
