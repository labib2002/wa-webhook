...
// PostgREST served timestamptz as full-microsecond ISO text. node-postgres
// builds a JS Date instead, which floors to milliseconds, and the inbox feeds
// updated_at straight back as ?since= — a floored watermark never excludes its
// own row, so every poll refetches it forever. Hand back ISO text at the
// stored precision.
pg.types.setTypeParser(1184, (v) => (v === null ? null : v.replace(' ', 'T').replace('+00', 'Z')));
