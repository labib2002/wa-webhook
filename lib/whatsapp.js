...
// Send a pre-approved template message (business-initiated sends outside the
// 24-hour window can ONLY be templates — Cloud API rule). `components` follows
// the Graph API shape: [{ type: 'body', parameters: [{ type: 'text', text }] }].
async function sendTemplate(toWaId, { name, language = 'en', components = [] }) {
  if (!isConfigured()) {
    return {
      ok: false,
      error: 'Sending is not configured. Set WHATSAPP_TOKEN and PHONE_NUMBER_ID.',
  ...
  if (!resp.ok) {
    // 132001 = template does not exist / not approved yet — the expected state
    // until Meta approves the submissions; surface it in plain words.
    const code = data && data.error && data.error.code;
    if (code === 132001) {
      return { ok: false, error: `Template '${name}' is not approved (or does not exist) for this WABA yet.`, raw: data };
    }
    return { ok: false, error: describeGraphError(resp.status, data), raw: data };
  }
  const waMessageId =
...
module.exports = { sendText, sendTemplate, sendMedia, uploadMedia, mediaCategory, markRead, isConfigured };
