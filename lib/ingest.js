  ...
  for (const entry of entries) {
    ...
    for (const change of changes) {
      ...
      for (const msg of value.messages || []) {
        ...
        // Customer deleted ("delete for everyone") — WhatsApp delivers this as an
        // 'unsupported' message carrying the original id. If we already have that
        // message, FLAG it as customer-deleted but KEEP its content (transparency
        // marker, not a redaction). If we don't have it, fall through and store a
        // normal placeholder.
        if (msg.type === 'unsupported' && msg.id) {
          ...
          if (orig) {
            await db
              .from('messages')
              .update({ deleted: true, deleted_by: 'customer', deleted_at: atIso })
              .eq('wa_message_id', msg.id);
            summary.messages++;
            continue;
          }
