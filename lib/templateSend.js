...
async function sendTemplateFlow(db, { waId, template, language, components, key }) {
  ...
  if (key && db) {
    ...
    if (found.row) {
      return { status: 200, body: { ok: true, deduped: true, id: found.row.id, waMessageId: found.row.wa_message_id || null } };
    }
    if (!found.skip) {
      ...
      if (r.existing) {
        return { status: 200, body: { ok: true, deduped: true, id: r.existing.id, waMessageId: r.existing.wa_message_id || null } };
      }
      if (r.conflict) {
