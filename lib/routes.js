...
router.get('/messages', async (req, res) => {
  ...
  try {
    ...
    let q = db
      .from('messages')
      .select('id, wa_message_id, wa_id, direction, type, body, media_meta, media_status, reaction, deleted, deleted_by, status, error, wa_timestamp, created_at')
      .eq('wa_id', wa_id)
      .order('created_at', { ascending: true })
      .order('id', { ascending: true })
      .limit(500);
    if (after && Number.isFinite(after)) q = q.gt('id', after);
