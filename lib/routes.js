...
router.get('/messages', async (req, res) => {
  ...
  // status ticks, deletes). Without it, the full thread loads.
  const since = req.query.since ? String(req.query.since) : null;
  try {
    const db = getDb();
    const withUpd = 'id, wa_message_id, wa_id, direction, type, body, media_meta, media_status, reaction, deleted, deleted_by, status, error, wa_timestamp, created_at, updated_at';
    ...
    if (error && /updated_at/.test(error.message || '')) {
      const cols = 'id, wa_message_id, wa_id, direction, type, body, media_meta, media_status, reaction, deleted, deleted_by, status, error, wa_timestamp, created_at';
...
// DELETE /api/messages/:id  -> agent deletes a message.
// Attempts to recall it on WhatsApp, removes the bucket file, and tombstones
// the row (keeps it for audit; clears the content). Returns whether the
// WhatsApp recall actually succeeded.
router.delete('/messages/:id', async (req, res) => {
  ...
  try {
    ...
    const { data: row, error } = await db
      .from('messages')
      .select('id, wa_message_id, direction, media_path, deleted')
      .eq('id', id)
      .maybeSingle();
    if (error) throw error;
    if (!row) return res.status(404).json({ error: 'Message not found.' });

    // Attempt WhatsApp recall (only meaningful for messages we sent).
    let recalled = false;
    if (row.direction === 'out' && row.wa_message_id) {
      const del = await wa.deleteMessage(row.wa_message_id);
      recalled = del.ok;
    }

    // Remove the stored media file, if any.
    if (row.media_path) {
      try { await db.storage.from(media.BUCKET()).remove([row.media_path]); } catch (_) {}
    }

    // Tombstone the row (audit trail; content cleared).
...
router.post('/send-media', async (req, res) => {
  const { wa_id, file_base64, mime, filename, caption } = req.body || {};
  if (!wa_id) return res.status(400).json({ error: 'wa_id is required.' });
  ...
  try {
    ...
    const preview = caption ? `${labelMap[category]} · ${caption}` : labelMap[category];
    ...
    const { data, error } = await db
      ...
      .insert({
        ...
        type: category,
        body: preview,
        media_meta: { filename: filename || null, mime_type: mime || null, caption: caption || null },
        media_path,
        media_status,
        status: 'sent',
        wa_timestamp: nowIso,
