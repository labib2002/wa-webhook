...
// Outbound already caps at 25 MB (lib/routes.js); inbound had no cap at all and
// buffers the whole object in memory, while WhatsApp allows documents to 100 MB.
// The Supabase bucket used to enforce this server-side; S3 has no equivalent.
const MAX_BYTES = () => parseInt(process.env.MEDIA_MAX_BYTES, 10) || 26214400;
...
function extFromMime(mime) {
  ...
  const table = {
    ...
    'audio/ogg': 'ogg', 'audio/webm': 'webm', 'video/webm': 'webm', 'application/pdf': 'pdf',
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document': 'docx',
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': 'xlsx',
    'text/plain': 'txt',
  };
  // A subtype is only a usable extension when it already looks like one.
  // Without this, application/octet-stream produced ".octetstream".
  const sub = (m.split('/')[1] || '').replace(/[^a-z0-9]/g, '');
  return table[m] || (sub && sub.length <= 5 ? sub : 'bin');
}

/** Documents arrive with their real name; its extension beats a generic mime. */
function extFromName(filename) {
  const m = /\.([A-Za-z0-9]{1,8})$/.exec(String(filename || '').trim());
  return m ? m[1].toLowerCase() : null;
}

/** A name the browser can save as, preferring what the sender actually called it. */
function downloadName(mediaMeta, mediaId, mime) {
  const original = mediaMeta && mediaMeta.filename;
  if (original && String(original).trim()) return String(original).trim();
  return `whatsapp-${mediaId || 'media'}.${extFromName(original) || extFromMime(mime)}`;
}
...
async function fetchAndStore(mediaId, waId, kind, filename) {
  ...
  if (declared > cap) {
    // ingest.js flips the row to 'failed', so the bubble degrades to its
    // labeled placeholder instead of the process dying on the allocation.
    throw new Error(`media ${mediaId} is ${declared} bytes, over the ${cap}-byte cap`);
  }
  const bytes = await downloadBytes(meta.url);
  if (bytes.length > cap) {
    throw new Error(`media ${mediaId} downloaded ${bytes.length} bytes, over the ${cap}-byte cap`);
  }

  const ext = extFromName(filename) || extFromMime(meta.mime_type);
...
async function signedUrl(path, expiresSeconds = 600, download) {
  const db = getDb();
  const { data, error } = await db.storage
    .from(BUCKET())
    .createSignedUrl(path, expiresSeconds, download ? { download } : undefined);
  if (error) throw new Error(error.message);
  return data.signedUrl;
}

module.exports = { fetchAndStore, signedUrl, extFromMime, extFromName, downloadName, BUCKET };
