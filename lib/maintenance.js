// Daily maintenance handler for GET /api/cron/maintenance, fired at 03:00 UTC
// by the box crontab (CRON_TZ=UTC) against 127.0.0.1. Registered in
// api/index.js BEFORE the passcode router; auth is its own, see authorized().
//
...
//   2) re-drive inbound media stuck at 'pending'/'failed' (lib/recovery.js).
//   3) prune login_attempts rows older than 24h.
//   4) usage measurement — total media bytes and messages row count.
//   5) usage alert — above 70% of MEDIA_CAP_MB / MESSAGES_CAP_ROWS, send one
//      WhatsApp text to WA_USAGE_ALERT_TO (skipped when unset / below 70%).
//
// Response JSON carries the numbers so the cron log shows them.
...
// One flat paginated listing. S3 has no folders, so the old breadth-first
// folder walk, its 20k-object cap and the truncated flag are all gone.
async function measureMediaBytes(db) {
  const { data, error } = await db.storage.from(media.BUCKET()).measure();
  if (error) throw error;
  return data;
}
...
module.exports = async function maintenance(req, res) {
  ...
  // 2) re-drive inbound media that never reached the bucket
  try {
    out.media_recovered = await recovery.recoverStuckMedia(db);
  } catch (e) {
    errors.push(`media recovery: ${(e && e.message) || e}`);
  }

  // 3) prune login_attempts
  try {
  ...
  // 5) usage alert (one per run, only above 70% of either cap).
  // WA_USAGE_ALERT_TO takes a comma-separated list of numbers.
  try {
    const capMb = envInt('MEDIA_CAP_MB', 1000);
    const capRows = envInt('MESSAGES_CAP_ROWS', 400000);
    const alertTos = (process.env.WA_USAGE_ALERT_TO || '')
      .split(',')
      .map((n) => n.replace(/\D/g, ''))
      .filter(Boolean);
    ...
    if ((mediaHot || rowsHot) && alertTos.length) {
      ...
      for (const to of alertTos) {
        // Recipients rarely have an open 24h session with the business number,
        // so free text is undeliverable most of the time — template first
        // (ops_storage_alert, pending Meta approval), free text as fallback.
        let sent = await wa.sendTemplate(to, {
          name: 'ops_storage_alert',
          language: 'en',
          components: [{
            type: 'body',
            parameters: [
              { type: 'text', text: `${mediaMb == null ? '?' : mediaMb} MB / ${capMb} MB` },
              { type: 'text', text: `${out.message_rows == null ? '?' : out.message_rows} / ${capRows}` },
            ],
          }],
        });
        if (!sent.ok) sent = await wa.sendText(to, text);
        if (sent.ok) out.alerts.push(`${text} -> ${to.slice(0, 4)}****`);
        else errors.push(`alert send to ${to.slice(0, 4)}****: ${sent.error}`);
      }
    }
  } catch (e) {
